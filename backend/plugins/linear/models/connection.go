...
const (
	DefaultEndpoint         = "https://api.linear.app"
	DefaultRateLimitPerHour = 3600
)

// LinearConn holds Linear connection settings.
...
func (conn *LinearConn) Sanitize() LinearConn {
	clone := *conn
	clone.Token = utils.SanitizeString(clone.Token)
	return clone
}

// LinearConnection persists connection details in the database.
...
func (c LinearConnection) Sanitize() LinearConnection {
	c.LinearConn = c.LinearConn.Sanitize()
	return c
}

func (c *LinearConnection) MergeFromRequest(target *LinearConnection, body map[string]interface{}) error {
...
func (c *LinearConnection) Normalize() {
	if c.Endpoint == "" {
